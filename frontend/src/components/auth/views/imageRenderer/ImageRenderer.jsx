...
const PdfThumbnail = ({ pdfUrl, thumbnailUrl, onLoad }) => {
  ...
  const getImageUrl = () => {
    // If we have a valid thumbnail URL that's a webp image (not a PDF), use it directly
    if (thumbnailUrl && (thumbnailUrl.includes('.webp') || thumbnailUrl.includes('.jpg') || thumbnailUrl.includes('.jpeg') || thumbnailUrl.includes('.png'))) {
      return thumbnailUrl;
    }
    
    // For PDFs, generate thumbnail using backend
    // Use thumbnailUrl if it's a full URL, otherwise use pdfUrl
    const pdfSource = thumbnailUrl && thumbnailUrl.startsWith('http') ? thumbnailUrl : pdfUrl;
...
const ImageRenderer = (props) => {
  ...
  const triggerPrint = useReactToPrint({
    ...
    onPrintError: (error) => {
      ...
      displaySnackbar({ message: 'Print failed. Please try again.' });
    }
  });
  
  // Wrapper function to handle print with login and credits check
  const handlePrint = async () => {
    ...
    if (!selectedItem?.url) {
      displaySnackbar({ message: 'No image selected for printing' });
      return;
    }
    
    // Don't allow printing PDFs directly - show message
    if (selectedItem?.fileType === 'pdf') {
      displaySnackbar({ message: 'Please open the PDF and use browser print for PDF files' });
      return;
    }
    
    // Deduct 1 credit for printing
    const creditResult = await deductCredits(1, 'print');
    ...
    doc.write(`
      ...
      <html>
        <head>
          <title>Print - ${selectedItem?.tittle || 'Image'}</title>
          <style>
        ...
        <body>
          <img src="${selectedItem.url}" alt="${selectedItem?.tittle || 'Image'}" />
        </body>
      </html>
    `);
    doc.close();
    ...
    if (img) {
      ...
      img.onerror = () => {
        displaySnackbar({ message: 'Failed to load image for printing' });
        document.body.removeChild(printFrame);
      };
    }
  };
  ...
  useEffect(() => {
    ...
    if (props.pageType === 'result') {
      ...
      const actualImageCount = displayableImages.length;
      
      // Always base pagination on ACTUAL loaded images, not totalCount from API
      // totalCount represents total in database, not what's loaded in current view
      if (actualImageCount === 0) {
        setPageCount(1);
      } else {
        setPageCount(Math.max(1, Math.ceil(actualImageCount / 21)));
      }
    }
  }, [props?.selectedFilters, props.data, pagination, props.pageType, props.totalCount])
                ...
                <div
                  ...
                  onClick={() => {
                    ...
                    if (props.place === "MyImages") {
                      ...
                      // For images and PDFs, oldData[key] contains the URL
                      imageUrl = oldData[key] || key;
                      // For PDFs, generate thumbnail URL from backend
                      if (fileType === "pdf") {
                        const backendUrl = process.env.REACT_APP_BACKEND_URL || '';
                        const encodedUrl = encodeURIComponent(imageUrl);
                        thumbnailUrl = `${backendUrl}/api/rest/images/pdf-thumbnail?url=${encodedUrl}&width=144&height=185`;
                      } else {
                        thumbnailUrl = oldData[key] || null;
                      }
                    }
                    handleImageResize({
                      url: imageUrl,
            ...
            >
              ...
              {selectedItem?.fileType === "pdf" ? (
                ...
                <TransformWrapper ref={zoomRef}>
                  <TransformComponent>
                    <div className="drawerImage" ref={printableDiv}>
                      <img
                        alt=""
