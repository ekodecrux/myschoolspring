...
const AdminPanel = () => {
  ...
  const handleBulkUpload = async () => {
    ...
    setBulkUploadResults(null); // Clear previous results before upload
    ...
    try {
      ...
      // Clear file on failure to allow re-upload with same or different file
      setBulkUploadFile(null);
      // Reset the file input element
      const fileInput = document.getElementById('bulk-upload-input');
      if (fileInput) fileInput.value = '';
      // Show error but don't clear results - show error in snackbar
      const errorMsg = error.response?.data?.detail || 'Upload failed';
      setBulkUploadResults({ errors: [{ row: 0, error: errorMsg }], success_count: 0 });
