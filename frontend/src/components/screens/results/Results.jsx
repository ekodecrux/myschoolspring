...
const Results = () => {
  ...
  React.useEffect(() => {
    // Use URLSearchParams for proper URL parameter parsing
    const params = new URLSearchParams(location?.search || '');
    const textParam = params.get('text');
    const searchQuery = textParam ? decodeURIComponent(textParam).toLowerCase() : "";
    ...
    if (searchQuery) {
      dispatch(
        ...
        // ImageRenderer expects { "path_key": "full_url", ... }
        const results = res.payload?.data || [];
        const transformedData = {};
        results.forEach((item) => {
          // Use the path as key (extract filename from URL) and full URL as value
          const fullUrl = item.path || item.thumbnail || '';
          if (fullUrl) {
            // Extract a short key from the URL for the grid
            const urlParts = fullUrl.split('/');
            const key = urlParts.slice(-2).join('/') || fullUrl; // Use last 2 parts as key
            transformedData[key] = fullUrl;
          }
        });
